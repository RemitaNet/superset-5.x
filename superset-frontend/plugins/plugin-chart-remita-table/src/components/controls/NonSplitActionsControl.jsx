            ...
            >
              ...
              <Button
                ...
                onClick={() => {
                  ...
                  for (const a of arr) {
                    if (!a || typeof a !== 'object') { message.error('Each action must be an object'); return; }
                    if (!a.key || typeof a.key !== 'string') { message.error('Each action must have a non-empty "key"'); return; }
                    if (!a.label || typeof a.label !== 'string') { message.error(`Action "${a.key}": missing "label"`); return; }
                    if (a.publishEvent === false && (!a.actionUrl || typeof a.actionUrl !== 'string')) { message.error(`Action "${a.key}": "actionUrl" is required when publishEvent is false`); return; }
                  }
                  if (editingIndex !== null) {
                    const updatedActions = actions.map((action, idx) =>
