...
const SplitActionsControl = ({
  ...
  const openAddModal = () => {
    openModal();
    // Set sensible defaults for new actions
    setTimeout(() => {
      try {
        form.setFieldsValue({
  ...
  const revealAddForm = () => {
    ...
    setEditingIndex(null);
    form.resetFields();
    // Default values when inline form is revealed
    setTimeout(() => {
      try {
        form.setFieldsValue({
          boundToSelection: false,
            ...
            <Button
              ...
              onClick={() => {
                try {
                  ...
                  for (const a of arr) {
                    if (!a || typeof a !== 'object') { const msg='Each action must be an object'; setValidationError(msg); message.error(msg); return; }
                    if (!a.key || typeof a.key !== 'string') { const msg='Each action must have a non-empty "key"'; setValidationError(msg); message.error(msg); return; }
                    if (!a.label || typeof a.label !== 'string') { const msg=`Action "${a.key}": missing "label"`; setValidationError(msg); message.error(msg); return; }
                    if (a.publishEvent === false && (!a.actionUrl || typeof a.actionUrl !== 'string')) { const msg=`Action "${a.key}": "actionUrl" is required when publishEvent is false`; setValidationError(msg); message.error(msg); return; }
                  }
                  if (editingIndex !== null) {
                    const updatedActions = actions.map((action, idx) =>
                      idx === editingIndex ? parsed : action
                    ...
                    message.success(`JSON saved (${count} action${count !== 1 ? 's' : ''})`);
                  } else {
                    setActions(parsed);
                    onChange(JSON.stringify(parsed));
                    const count = Array.isArray(parsed) ? parsed.length : 1;
